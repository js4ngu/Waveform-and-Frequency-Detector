...
float voltage_origin;
float voltage_origin_div = 0;
float voltage_origin_prev = 0;
float buffer[3] = {0,};
int wave = 4;
...
int main(void) {
    ...
    for (int i = 0; i < sampling_rate; i++) {
//        voltage_origin = dc_sig(3);
//        voltage_origin = sin_sig(2,1, i);
        voltage_origin = triangle_sig(2, 1, i);
        voltage_origin_div = voltage_origin - voltage_origin_prev;

        shiftLeft(buffer, 1, 3);

        buffer[2] = voltage_origin_div;
        ...
        //조건 판별부
        voltage_origin_prev = voltage_origin;
